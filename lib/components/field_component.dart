...
class FieldComponent extends Component {
  ...
  void _renderTapRing(Canvas canvas, HexLayout layout) {
    ...
    final radius = game.tuning.tapRadius;
    const segments = 34;
    _stroke
  ...
  void _renderTutorialTarget(Canvas canvas, HexLayout layout) {
    ...
    final pulse = 0.5 + 0.5 * math.sin(game.elapsed * 4.5);
    _stroke
    ...
    for (final pickup in game.pickups) {
      ...
      switch (pickup.kind) {
        case PickupKind.treat:
          ...
          canvas.translate(centre.dx, centre.dy);
          canvas.rotate(-0.38);
          canvas.drawPath(_bonePath(size * 1.7), _fill);
          canvas.restore();
        ...
        case PickupKind.radiusPlus:
          ...
          for (final r in const [0.45, 0.8, 1.15]) {
            canvas.drawCircle(centre, size * r, _stroke);
          }
        ...
        case PickupKind.blast:
          ...
          canvas.drawCircle(centre, size * 0.26, _fill);
        case PickupKind.dig:
  ...
  void _renderGoal(Canvas canvas, HexLayout layout) {
    ...
    canvas.drawPath(_bonePath(layout.size), _fill);
    canvas.restore();
  }

  static Path _bonePath(double size) {
