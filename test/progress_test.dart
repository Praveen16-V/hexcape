...
void main() {
  ...
  group('Progress', () {
    ...
    test('the furthest endless clear is the only endless progress', () async {
      final progress = await Progress.load();
      await progress.recordEndlessClear(level: 94);
      expect(progress.endlessBest, 94);

      await progress.recordEndlessClear(level: 90);
      expect(progress.endlessBest, 94, reason: 'the furthest still stands');
      expect(progress.recordFor(94).played, isFalse);
      expect(progress.totalStars, 0);
      expect(progress.unlocked, 1);
    });
