...
void main() {
  group('The campaign, end to end', () {
    test('every level generates, and every level has a way through', () {
    ...
    test('no level demands provably optimal play', () {
    ...
    test('the climb never goes backwards', () {
      // Every band picks up where the last left off. A level that is easier
      // than the one before it reads as the game losing its nerve.
      ...
      for (var n = 1; n <= Campaign.length; n++) {
        ...
        if (rules.budget) {
          expect(
            ...
            reason: 'level $n hands back taps',
          );
        ...
        if (rules.hunger) {
          expect(
            ...
            reason: 'level $n hands back time',
          );
        ...
        expect(
          ...
          reason: 'level $n thins the walls',
        );
        anchors = rules.anchorDensity;
      }
    }, timeout: const Timeout(Duration(minutes: 4)));

    test('the tutorial cannot be lost on its budget', () {
    ...
    test('no level traps the floor player with no route at all', () {
    ...
    test('the campaign as it stands, in numbers', () {
